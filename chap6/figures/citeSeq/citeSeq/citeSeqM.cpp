...
{
    int nS, status,nOut,flag, maxFileName=30;
    char* refIn, *outfile,*refOut,* citeSeqFile;

    if (nlhs != 0) { Usage(); return; } // 1 lhs Output

    if (nrhs != 1) { Usage(); return; } // 1 rhs Input

    /* Check to be sure input argument is a string. */
    if (!(mxIsChar(prhs[0]))) mexErrMsgTxt(" Input must be of type string.");
    nS = (mxGetM(prhs[0]) * mxGetN(prhs[0])) + 5;
	nS = 50;
    //s = mxCalloc(nS, sizeof(char));
    refIn = (char*)malloc(nS * sizeof(char));
    status = mxGetString(prhs[0], refIn, nS);
    printf(" input file %s\n", refIn);
    if (status != 0)  mexWarnMsgTxt(" Not enough string length allocated. String is truncated.");
    ...
    strcpy_s(citeSeqFile, maxFileName, "citationSeq.txt");
    if (nS > 0)
    {
        flag = citeOrder(refIn, refOut, citeSeqFile);
		if (flag < 0) {
			fclose(fpRefOut);
			fclose(fpRef);
			fclose(fpKey);
...
void Usage()
{
    printf(" Kaluri V Rangarao ordering of refrences 10th Jul 2025\n");
    printf(" updated Prof Kaluri V Rangarao Analasis of .lp file and convert 15th Mar 2025\n");
    printf(" usage dos prompt or MATLAB prompt\n");
    printf(" 1. dos usage \n");
    printf(" 2. MATLAB usage --> out=cad2texM('f2_1.lp')\n");
    printf(" Copy Right can contact me at kaluri@ieee.org\n");
    printf(" cad2tex\n");
    return;
}
...
int citeOrder(char* refIn, char* refOut, char* citeseq)
{
	...
	if (fpRefOut == NULL) {
		...
		mexWarnMsgTxt(" Error opening out file.\n");
		fclose(fpRefOut);
		return -1;
	}
...
int passA(char* refFile, char** A, int* lineBib, int* maxlines)
{
	...
	if (fpRef == NULL) {
		printf("!. Error opening file %s(%d)\n", refFile, fEr);
		mexWarnMsgTxt("Error opening reference file\n");
		return -1;
	}
	...
	while (!(feof(fpRef)))
	{
		...
		if (k > lineMax)
		{
			...
			printf("---------------------------------------------------------------------\n.\n.\n");
			fclose(fpRef);
			mexWarnMsgTxt("  FATAL ERROR CONDITION\n");
			return -1;
		}
		if (endOfLine > 0)
		{
...
int getKey(char* citeSeqFile, char** A)
{
	...
	if (fpRef == NULL) {
		printf("!. Error opening file %s(%d)\n", citeSeqFile, fEr);
		mexWarnMsgTxt(" Error opening citation sequence file.\n");
		return -1;
	}
			...
			if (strlen(line) > 0)
			{
				...
				for (i = 0; i < 10; i++)
				{
					if (line[i] != 32) // remove blanks
					{
						A[indxA][p] = line[i];
						p = p + 1;
						if (p > 19) mexWarnMsgTxt(" memory violation.\n");
					}
				}
				A[indxA][p] = 0;
				indxA = indxA + 1;
