...
{
//  Copy Right Protected  by Prof Kaluri
//  1st May 2017   .
	complex c;
	c.r = a.r - b.r;
	c.i = a.i - b.i;
...
//	printf("!. I had problems and fixed and written in C\n");
	...
	for (i = 0; i < N; i++) x[i] = (cabs.cabs(X[i]) / Nr)*0.5F; 
// on 10th Jun 2018 Princeton NJ .. verified with matlab FFT
//	{
//		xr = X[i].r; xi = X[i].i; 
//		x[i] = (sqrt(xr*xr + xi*xi))*(1.0 / Nr);
...
int* Low2HighComplexOnReal(int M, complex* z)
{
    double *zr;
    complex *temp;
    int k, *indx;
    zr = (double*)malloc(M * sizeof(double));
    temp = (complex*)malloc(M * sizeof(complex));
    for (k=0; k<M; k++) { temp[k] = z[k]; zr[k] = z[k].r;}
    indx = Low2High(M,zr);
    for (k=0; k<M; k++) { z[k] = temp[indx[k]]; }
    return indx;
}
...
void myPrintfX(int* JkC, int N, int perLine)
{
	int k, remPerLine;
	printf("void myPrintfX()\n");
	//  Copy Right Protected  written by Prof Kaluri
	for (k = 0; k < N; k++)
	{
		remPerLine = (k + 1) % perLine;
		printf("%x ", JkC[k]); if (remPerLine == 0) printf("[%d]\n", k);
	}
	return;
}
...
{
	double uk1 = 0, uk0 = 0;
	double xk1 = 0, xk0 = 0, zk = 0;
	double zk1 = 0, zk0 = 0;
	double ek = 0, pk = 0.0, rk,delRk;
	double *dly, *bufer;
//    double loopGain = 0.01F, shift = 0.0;
	double loopGain = 0.05F, shift = 0.0;
...
//	printf("!. creating 90 deg phase shift using orthogonal decomposer-31stAug2023\n");
	for (k = 0; k < maxk; k++) { u[k] = z[k].r; }
	uk0 = 0.0; xk1 = 0.0; pk = 0.0; uk1 = 0.0; rk = 0.1F;
	for (k = 0; k < maxk; k++)
	{

		uk1 = uk0; xk1 = xk0; zk1 = zk0;
		uk0 = u[k]; // input sequence uk 
...
)
{
	...
	sigma[0] = 1.0; mu[0] = 1.0;
	switch (lenL)
	{
	case 0: flag = -2; printf("!. DATA Length zero .. in stdMuLin %d\n", flag);  break;
...
//		printf("sum of the data %3.4f\n", sumB);
		muA = sumB / (double)(lenL);
		sumA = 0.0;
		for (i = 0; i < lenL; i++) sumA = sumA + (localL[i] - muA) * (localL[i] - muA);
		sigmaA = sqrt(sumA / (double)(lenL - 1));
//      important bug fix 11th Sep 2023 at Dublin Ohio        
        if (sigmaA < 1.0E-8)
		{
			printf("  (cMath) No Energy in Time Series[%d] std %5.3e\n", lenL, sigmaA); flag = -1;
		}
		else flag = 1;
...
double* lineCluster(int s, int e, double* X, int lenX, double* stdMu)
{
	double* pieceX;
	int lenP = e - s + 1;
	int k, indx, flag;
	...
	if (e < s) return NULL;
	pieceX = (double*)malloc(lenP * sizeof(double));
	for (k = 0; k < lenP; k++)
	{
		indx = s + k;
		pieceX[k] = X[indx];
	}
	flag = stdMuLin(pieceX, &stdMu[0], &stdMu[1], lenP);
	//free(pieceX);
	return pieceX;
}
