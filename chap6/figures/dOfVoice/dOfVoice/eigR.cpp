...
//	printf("!. Ar input real symmetric Matrix, **P Output Transformation Matrix, **a Output Tridiagonal  Matrix, nA size(A)\n");
	...
	u = (double *)malloc(nA * sizeof(double) );
	w = (double *)malloc(nA * sizeof(double) );

	Q = (double **)malloc(nA * sizeof(double *));
	for (m = 0; m < nA; m++) Q[m] = (double *)malloc(nA * sizeof(double) );
	mul = (double **)malloc(nA * sizeof(double *));
	for (m = 0; m < nA; m++) mul[m] = (double *)malloc(nA * sizeof(double) );
	res = (double **)malloc(nA * sizeof(double *));
	for (m = 0; m < nA; m++) res[m] = (double *)malloc(nA * sizeof(double) );
	res1 = (double **)malloc(nA * sizeof(double *));
	for (m = 0; m < nA; m++) res1[m] = (double *)malloc(nA * sizeof(double) );
...
//	printf("!. Converting into tri-diagonal form\n");
	...
	for (m = 0; m<nA - 2; m++)
	{
		...
		/* printf("w =");
		// for (i = 0; i <nA ; i++) printf("%f ",w[i]); printf("\n"); */
...
//	int i, j, ir, jc, kr, irv, irn, togo, its, m, k;
	...
	double TOL = 1.0E-15, shift, trace, det, oldb, disc, r, temp1, temp2, mu1, mu2, sadd;
