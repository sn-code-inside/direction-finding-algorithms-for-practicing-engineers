...
//			printf("[%d.%d] (%3.4f %3.4f)\n", i,bins[i], steering[i][k].r,steering[i][k].i);
		}
	}
...
double * rgAvg(double ** rgMat, double *config)
{
	int i, k, numberOfTgts, cby2;
	double* rg;
	numberOfTgts = (int)(config[4]);
	cby2 = (int)(config[13])/2;
	rg = myAlloc(numberOfTgts);
	for (i = 0; i < numberOfTgts; i++)
	{
		rg[i] = 0;
		for (k = 0; k < cby2; k++) rg[i] = rg[i] + rgMat[i][k];
		rg[i] = rg[i] / (double(cby2));
	}
	return rg;
}
