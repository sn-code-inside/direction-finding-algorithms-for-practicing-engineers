...
//  Copy Right Protected Dr Kaluri Reserach House written by Prof Kaluri
//  15th Feb 2017   Dr Kaluri Reserach House.
...
void MatrixInversion(double **A, int order, double **I)
{
	...
	printf("!. start Gauss Jordan row operations on matrix order %d\n",order);
	for (i = 0; i<order; i++)								//automatically initialize the unit matrix, e.g.
		for (j = 0; j<order; j++)							//	-       -
			...
			printf("!. end of Gauss Jordan iterations\n");
...
)
{
	...
	switch (lenL)
	{
	case 0: flag = -2;  printf("!. NULL DATA[%d] .. in stdMuLin %d\n", lenL, flag);  break;
...
int lineCluster(int s, int e, double* X, int lenX, double* stdMu)
{
	double* pieceX;
	int lenP = e - s + 1;
	int k, indx, flag;
