...
[thetaHat(k) ulaPks] = map3ulaR(thetaA, thetaB, thetaC);
%% plot A
    if (k==ten) 
...
s1 = ['$\theta^\circ$ = ' num2str(p(1),3) '$\hat\theta^\circ$ ' ...
    si num2str(abs(p(2)),3)];
doaSampl = doa(1:5:swpMax);
subplot(311); plot(doa,thetaHat,'-.r');grid
hold on; plot(doaSampl,doaSampl,'sb','MarkerSize', 3); hold off;
hold on; plot(doa(ten),thetaHat(ten),'ob','MarkerSize', 4,...
    'MarkerFaceColor','k'); hold off;
title(['$r=$ ' num2str(r,3) ' SNR [' num2str(snrTen,3) ']dB for 3-ULA ' num2str(m,3) ...
   ' at ' num2str(doa(ten),3) '$^\circ$'],'interpreter', 'latex');
...
%% label line
  indx = 40;
  xa = [doa(indx)-10 doa(indx)];
  ya = [thetaHat(indx)  thetaHat(indx)];
