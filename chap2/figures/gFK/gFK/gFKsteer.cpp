...
//	printf("void rgSteering()\n");
	...
	cby2 = rc[1] / 2;
	pi = atan(1.0)*4.0;
	printf("computing steering vector size of complex A %d by %d\n",rc[0],cby2);
	FILE* fpgFK; // this introduced for book writing
	er = fopen_s(&fpgFK, "gFKobj.csv", "w"); // this introduced for book writing
	if (fpgFK == NULL) { printf("!. Failed to open file erKmusic.csv\n");  exit; } // this introduced for book writing
	config[14] = ngs;
//	..............................................................
...
//	..............................................................
	...
	for (k = 0; k < cby2; k++)
	{
		...
		for (i = 0; i < numberOfTgts; i++)
		{
			...
			if (i==0) printgFK(fpgFK,jkG, FnG, phase, ngs);
			steering[i][k]= abLocal;
			rgMat[i][k] = phasor;
...
//			printf("Amplitude at f=%3.4f is %3.4f and phase %3.4f\n", rgAvg[i],Amp,phi);
			w.r = Amp * cos(phi * pi / 180.0); w.i = Amp * sin(phi * pi / 180.0);
//			printf("steering vector r.%3.4f i.%3.4f\n", w.r, w.i);
			steering[i][k] = w;
		}
	}
...
//	Free memory ..
	fclose(fpgFK); // this introduced for book writing
	free(u); free(uScale);
	free(fn); free(jkG);
	free(FnG); free(phase);
	free(gainCh);
...
//		fn[i] = aa;  jK[i] = fabs(fa);
	...
	printf("!. Fn %3.5f Amp %3.5f Phase %3.5f\n", ab[0], ab[1], ab[2]);
	B[0].r = ab[1] * cos(ab[2] * pi / 180.0); B[0].i = ab[1] * sin(ab[2] * pi / 180.0);
//	printf("!. complex phasor r.%3.4f i.%3.4f\n", B[0].r, B[0].i);
	phasor[0] = ab[0];
//	donot delete this .. 
	return qTgt;
}
...
//	printf("double ObjFn()\n");
	...
	//  Copy Right Protected  written by Prof Kaluri
	//  15th Feb 2017   .
	//  Accelerator .. without Sine or Cosine .. CONFIDENTIAL ..
	//        thetaP = 0.5F*p;
	//        theta = acos(thetaP); 
...
void printgFK(FILE *fpgFK, double *jkG, double *FnG, double *phase, int ngs)
{
	int k;
	for (k = 0; k < ngs; k++)
	{
		printf("%3.4f %3.4f %3.4f\n", jkG[k], FnG[k], phase[k]); 
		fprintf(fpgFK, "%3.4f, %3.4f, %3.4f\n", jkG[k], FnG[k], phase[k]);
	}
	return;
}